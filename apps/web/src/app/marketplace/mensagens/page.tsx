...
import { createMarketplaceChatSocket } from '@/lib/socket';
  ...
  return (
    <div className="divide-y divide-border overflow-y-auto">
      ...
      {conversations.map((c) => {
        ...
        const peerName = isBuyer ? c.seller?.name : c.buyer?.name;
        ...
        return (
          ...
          >
            <span className="font-medium text-ink">{peerName ?? 'Conversa'}</span>
            <span className="truncate text-xs text-foreground-muted/80">{c.product?.title}</span>
            {lastMessage && (
...
function Thread({ conversationId, onBack }: { conversationId: string; onBack: () => void }) {
  ...
  const { data: conversations } = useQuery<MarketplaceConversation[]>({ queryKey: ['marketplace-chat', 'conversations'] });
  const conversation = conversations?.find((c) => c.id === conversationId);
  const isBuyer = conversation?.buyerId === user?.id;
  const peerName = (isBuyer ? conversation?.seller?.name : conversation?.buyer?.name) ?? 'Conversa';

  const { data: messages } = useQuery({
    queryKey: ['marketplace-chat', 'messages', conversationId],
    queryFn: async () => (await api.get<MarketplaceMessage[]>(`/marketplace-chat/conversations/${conversationId}/messages`)).data,
  });

  useEffect(() => {
    if (!token) return;
    const socket = createMarketplaceChatSocket(token);
    socketRef.current = socket;

    socket.emit('join', { conversationId });

    socket.on('message', (message: MarketplaceMessage) => {
      if (message.conversationId !== conversationId) return;
      queryClient.setQueryData<MarketplaceMessage[]>(['marketplace-chat', 'messages', conversationId], (old) =>
        old ? [...old, message] : [message],
      );
      queryClient.invalidateQueries({ queryKey: ['marketplace-chat', 'conversations'] });
    });
        ...
        <div>
          ...
          {conversation?.product && (
            <p className="text-xs text-foreground-muted">
              {conversation.product.title} · R$ {conversation.product.price.toFixed(0)}
            </p>
          )}
        </div>
      </div>
...
function MarketplaceMensagensContent() {
  const { user, token } = useAuthStore();
  ...
  const { data: conversations, isLoading } = useQuery({
    queryKey: ['marketplace-chat', 'conversations'],
    enabled: !!token,
    queryFn: async () => (await api.get<MarketplaceConversation[]>('/marketplace-chat/conversations')).data,
  });
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 sm:py-12">
      <h1 className="font-display mb-6 text-2xl text-ink">Mensagens do marketplace</h1>
...
export default function MarketplaceMensagensPage() {
  return (
    <Suspense fallback={null}>
      <MarketplaceMensagensContent />
    </Suspense>
  );
}
